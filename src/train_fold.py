...
Called as a subprocess by ``scripts/run_nested_cv.py`` (one job per GPU); can also
be run standalone for debugging. Two modes:
由 ``scripts/run_nested_cv.py`` 以子程序方式呼叫（一個 job 一張 GPU），也可單獨執行除錯。兩種模式：

  * ``--mode inner --inner-fold K``: train on the inner train split, validate on the
    inner val split, record the best validation AUC (used to rank HPs). No model saved.
    在內層訓練集上訓練、在內層驗證集上驗證，記錄最佳驗證 AUC（用來排序超參數）。不存模型。

  * ``--mode refit``: train on the outer pool's refit-train with early stopping on
    refit-val, then predict the held-out outer test fold. Saves per-sample probs+metrics.
    在外層池的 refit-train 上訓練、用 refit-val 做 early stopping，再預測被隔離的外層測試折。

The GPU is chosen by the caller via ``CUDA_VISIBLE_DEVICES`` (so this script uses ``cuda:0``).
...
def set_seed(seed):
    """Seed all RNGs for reproducibility. / 固定所有亂數來源，確保可重現。"""
    random.seed(seed)
    np.random.seed(seed)
...
# bfloat16 autocast: A100 has native bf16 with fp32 exponent range, so it is stable
# here (plain fp16 overflows this SwinUNETR -> NaN logits) and needs no GradScaler.
# bfloat16 autocast：A100 原生支援 bf16，指數範圍同 fp32，數值穩定（純 fp16 會溢位 → logits 變 NaN），且不需 GradScaler。
AMP_DTYPE = torch.bfloat16


@torch.no_grad()
def evaluate(model, loader, device):
    """Return (labels, probabilities) over a loader. / 對一個 loader 回傳 (標籤, 機率)。"""
    model.eval()
    ys, ps = [], []
    for vol, label in loader:
        ...
        prob = torch.sigmoid(logit.float())  # logits -> probability / logits → 機率
...
def train_one(dataset, train_idx, val_idx, hp, device, *, epochs, patience,
              ...
              tb_dir=None):
    """Train with early stopping on validation AUC; return a result dict.
    ...
    (the paper used effective batch 4, as 1/GPU across 4 GPUs).
    ``batch_size`` 是單卡的每步批次；透過 ``accum_steps`` 做梯度累積，有效批次 =
    ``batch_size * accum_steps``（論文用有效批次 4，是以 4 卡各 1 筆達成）。
    """
    set_seed(seed)
    ...
    # Build model -> load pretrained weights -> apply freeze strategy.
    # 建模 → 載入預訓練權重 → 套用凍結策略。
    ...
    # Only hand the trainable params to the optimizer (frozen ones are not updated).
    ...
    writer = SummaryWriter(tb_dir) if tb_dir else None  # TensorBoard: loss/AUC curves / 記錄 loss / AUC 曲線
    ...
    for epoch in range(epochs):
        ...
        for step, (vol, label) in enumerate(train_loader):
            ...
            with torch.autocast(device_type="cuda", dtype=AMP_DTYPE):
                logit = model(vol).squeeze(1)
                loss = criterion(logit, label) / accum_steps  # divide by accum steps / 除以累積步數
            loss.backward()
            running += loss.item() * accum_steps
            # Step the optimizer every accum_steps, or on the last batch.
            # 每累積 accum_steps 步、或到最後一個 batch 才更新權重。
            if (step + 1) % accum_steps == 0 or (step + 1) == n_batches:
                optimizer.step()
                optimizer.zero_grad()

        # Compute validation AUC once per epoch. / 每個 epoch 結束算一次驗證 AUC。
        ...
        if writer is not None:
            writer.add_scalar("train/loss", tl, epoch)        # training-loss curve / 訓練損失曲線
            if not np.isnan(val_auc):
                writer.add_scalar("val/auc", val_auc, epoch)  # validation-AUC curve / 驗證 AUC 曲線

        # Improved AUC -> update best (optionally keep weights); else bump patience.
    ...
    if keep_best_model and best_state is not None:
        model.load_state_dict(best_state)  # restore best weights for testing / 還原到最佳權重供後續測試
    return result, model
...
def main():
    ...
    ap.add_argument("--out", required=True, help="output path for the result JSON / 結果 JSON 的輸出路徑")
    args = ap.parse_args()
    ...
    # TensorBoard log dir: results/<exp>/tb/<mode>/<job name> (one run per job).
    ...
    # Shared train_one args (identical for both modes). / train_one 共用的參數（兩種模式都一樣）。
    ...
    if args.mode == "inner":
        # inner: train+validate, report only best val AUC (reduced epoch budget).
        ...
        # refit: train with the full epoch budget, then evaluate on the held-out test fold.
        ...
        # Also log this fold's test metrics to TensorBoard (same run as the curves).
        # 把該折的測試指標也寫進 TensorBoard（與訓練曲線同一個 run）。
        w = SummaryWriter(tb_dir)
        for k, v in test_metrics.items():
            if v == v:  # not nan / 非 nan
