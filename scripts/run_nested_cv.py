...
* HPs are tuned in *stages* (lr -> weight_decay -> dropout): each stage tunes one
  parameter while freezing the winners of previous stages.
  超參數以「階段」方式調（lr -> weight_decay -> dropout）：每階段只調一個參數、並把前面階段的勝出值固定。
* Stages run in lockstep across all outer folds. Within a stage, every inner job
  (outer_fold x candidate_value x inner_fold) is independent and dispatched
  concurrently -- one job per GPU.
  各階段在所有外層折之間「同步」進行。同一階段內每個內層 job（外層折 x 候選值 x 內層折）獨立，並行派發（一卡一 job）。
* Every job writes a result JSON; the orchestrator is fully resumable (a job whose
  result file exists is skipped).
  每個 job 都會寫出結果 JSON；協調器完全可續跑（結果檔已存在的 job 會被略過）。
...
def hp_tag(hp):
    """Turn a hyper-parameter set into a filename-safe tag. / 把一組超參數轉成檔名安全的字串標籤。"""
    return f"lr{fmt(hp['lr'])}_wd{fmt(hp['weight_decay'])}_do{fmt(hp['dropout'])}"


def job_out_path(jobs_dir, outer, mode, hp, inner=None):
    """Deterministic result path from (outer, mode, hp) -- the key to resumability.
...
def is_done(path):
    """Done = result file exists and parses as JSON. / 結果檔存在且能成功解析 JSON 才算完成。"""
...
def make_job(cfg_path, splits_path, outer, mode, hp, out_path, inner=None, log_path=None):
    """Build a job spec that invokes src.train_fold as a subprocess. / 組出一個呼叫 src.train_fold 子程序的工作描述。"""
...
def run_jobs(jobs, gpus, poll=5.0):
    """Run a batch of jobs concurrently (one per GPU), skipping finished ones.
    ...
    while queue or running:
        # Dispatch pending jobs to free GPUs. / 把待辦 job 派發到空閒的 GPU。
        for gpu in gpus:
            if gpu in running or not queue:
                continue
            job = queue.pop(0)
            # CUDA_DEVICE_ORDER=PCI_BUS_ID makes CUDA's device indices match nvidia-smi,
            # so the config's GPU ids select the intended cards (CUDA's default
            # FASTEST_FIRST order can otherwise map an id onto the small display GPU -> OOM).
            # CUDA_DEVICE_ORDER=PCI_BUS_ID 讓 CUDA 編號與 nvidia-smi 一致，config 的 GPU id 才會選到正確的卡
            #（否則預設 FASTEST_FIRST 順序可能把某個 id 對應到小顯示卡而 OOM）。
        ...
        # Poll finished jobs and free their GPU for the next one. / 輪詢已結束的 job，釋放其 GPU 供下一個使用。
        time.sleep(poll)
...
def mean_inner_auc(jobs_dir, outer, hp, inner_folds):
    """Mean inner-validation AUC over all inner folds for one (fold, hp) -- ranks HPs.
...
def aggregate(cfg, splits, jobs_dir, out_dir):
    """Collect refit/test results into per-fold + pooled summaries. / 把各折的 refit/測試結果彙整成 逐折 + 彙總 的摘要。"""
    selected, per_fold, pooled_true, pooled_score = [], [], [], []
    for fold in splits["folds"]:
        o = fold["outer_fold"]
        # Find this fold's refit result (should be exactly one). / 找這一折的 refit 結果（正常恰好一個）。
        ...
        pooled_true += res["test"]["y_true"]      # pool all folds' test predictions / 彙總所有折的測試預測
        pooled_score += res["test"]["y_score"]

    summary = {"experiment": cfg["experiment_name"], "n_outer_folds": len(per_fold),
               "selected_per_fold": selected}
    if per_fold:
        summary["across_folds_mean_ci"] = summarise_across_folds(per_fold)        # mean±CI across folds (paper) / 跨折平均±CI（論文用）
...
def main():
    ...
    ap.add_argument("--dry-run", action="store_true", help="print the plan, run nothing / 只印出計畫、不實際執行")
    ap.add_argument("--only-aggregate", action="store_true", help="just rebuild summary.json / 只重建 summary.json")
    ap.add_argument("--max-parallel", type=int, default=None, help="override GPU count / 覆寫使用的 GPU 數")
    args = ap.parse_args()
    ...
    # Per-fold running "best" HPs, refined stage by stage. / 每個外層折各自維護「目前最佳超參數」，隨階段逐步精修。
    ...
    # ---- staged inner search, in lockstep across folds / 分階段內層搜尋，所有折同步進行 ----
    for si, stage in enumerate(stages):
        param, values = stage["name"], stage["values"]
        print(f"\n### Stage {si+1}/{len(stages)}: tuning '{param}' over {values}")
        batch = []
        # Collect this stage's inner jobs: all folds x all candidate values x all inner folds.
        # 收集這一階段「所有折 x 所有候選值 x 所有內層折」的內層 job。
        for fold in splits["folds"]:
            o = fold["outer_fold"]
            for val in values:
                hp = dict(best[o]); hp[param] = val      # replace the param under tuning on this fold's current best / 在該折目前最佳上替換正在調的參數
                for k in range(inner_folds):
        ...
        # Pick this stage's winning value per fold (highest mean inner AUC).
    ...
    # ---- refit with selected HP and evaluate on outer test folds / 以選定超參數 refit，並在外層測試折上評估 ----
